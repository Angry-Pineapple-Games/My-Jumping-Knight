...
class UserLogin(Resource):
    def post(self):
        ...
        if UserModel.verify_hash(data['password'], current_user.password):
            list= {"access_token": create_access_token(identity=data['username']),
                   "mylevel1": current_user.level1, "mylevel2": current_user.level2,
                   "mylevel3": current_user.level3}
            ...
            return {"msg": LLaveAsimetrica.myEncryptExternalPublicKey(json.dumps({"error": "Something went wrong"}), data['modulus'], data['exponent'])}
