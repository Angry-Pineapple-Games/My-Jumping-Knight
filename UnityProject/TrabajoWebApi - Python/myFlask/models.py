...
class UserModel(db.Model):
    ...
    def getLevel_by_rank(cls, rank1="", rank2="", rank3=""):
        ...
        result = ""
        if rank1 != "":
            for l in cls.query.filter_by(rank1=rank1).order_by(func.random()).limit(num).all():
                result += l.level1 + ";"
        elif rank2 != "":
            for l in cls.query.filter_by(rank2=rank2).order_by(func.random()).limit(num).all():
                result += l.level2 + ";"
        elif rank3 != "":
            for l in cls.query.filter_by(rank3=rank3).order_by(func.random()).limit(num).all():
                result += l.level3 + ";"
